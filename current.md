...
* [Experience Platform e amministrazione](#platform)
...
## [!UICONTROL Experience Platform] e amministrazione {#platform}
...
| -----------| ---------- | ---------- |
| Supporto per scostamenti fuso orario nello storico | 8 agosto 2019 | Analytics ora gestirà automaticamente gli scostamenti di fuso orario per gli hit con marca temporale. A seguito di questa modifica il 8 agosto, non sarà più necessario regolare i sistemi che caricano nei dati per l'elaborazione storica prima di inviarli nei dati. |
...
* (Inserzionisti con il servizio di tracciamento conversione di Advertising Cloud) Con Apple Intelligent Tracking Prevention (ITP) 2.2, rilasciato a maggio, i cookie di tracciamento conversione di Advertising Cloud vengono eliminati automaticamente dai browser Apple Safari dopo 24 ore. Advertising Cloud dispone, tuttavia, di una nuova soluzione ITP che consente di monitorare le conversioni che si verificano in Safari per oltre 24 ore dal clic originale. La soluzione utilizza l'archiviazione locale e la tecnologia iframe. Contatta il responsabile account Search Cloud Search per le istruzioni sull'implementazione.
* In Cerca &gt; Avanzate (ACM), ora è possibile configurare i suffissi URL finali a livello di campagna per i modelli di annunci di testo Google e per annunci acquisti.
* Gli inserzionisti con account Google Ads idonei per la corrispondenza cliente possono ora effettuare le seguenti operazioni:
   * Crea un pubblico customer ads di Google Ads utilizzando gli ID utente provenienti da un segmento di audience Adobe. Per visualizzare questa funzione, l'account pubblicitario deve essere configurato per consentirlo.
   * Crea un pubblico di Google Ads per un cliente caricando un file di dati cliente. Il file può contenere informazioni di contatto (indirizzi e-mail, indirizzi e-mail o numeri di telefono), ID utente o ID dispositivo mobili. Alcuni tipi di informazioni di contatto devono essere hash utilizzando l'algoritmo SHA -256.
   * Aggiorna qualsiasi pubblico di Google Match ad eccezione di audience create da un pubblico Adobe. Potete caricare dati per aggiungere, eliminare o sostituire tutti i dati esistenti per l'audience. Tutte le informazioni di contatto devono essere hash utilizzando l'algoritmo SHA -256.
* Le visualizzazioni Pubblico &gt; Destinazioni e Pubblico &gt; Esclusioni includono la colonna Tipo.
