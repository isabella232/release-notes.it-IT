...
* È stato risolto un problema nella definizione [!UICONTROL Attivatore] per l&#39;utilizzo corretto della clausola «like». (MCUI-6611)
...
| -----------| ---------- |  
| Segmentazione per conteggi distinti | Ora puoi creare segmenti in base a un numero specifico di elementi all’interno di una dimensione. Esempio: “Visitatori che hanno visualizzato più di 5 prodotti distinti” o “Visite in cui sono state visualizzate più di 5 pagine distinte”. (La funzione Approssimativa Count Distinct è ancora disponibile in Metriche calcolate, se si desidera creare una metrica distinta di una dimensione (ad es. # of customers, # of products, etc)). |
| Supporto Analisi ad hoc per Java 11 | Ad Hoc Analysis è ora compatibile con Java 11. Di seguito sono riportati alcuni aspetti relativi all&#39;utilizzo di Java 11. |
...
* Users now see an error message when they attempt to create an [!UICONTROL Algorithmic Model] using as baseline a trait type different from rule-based, segment, or onboarded. Il messaggio di errore riporta un testo simile al seguente: “È possibile usare come base di riferimento solo le caratteristiche basate su regole, di onboarding o segmenti”. (AAM-45235).
* La [!UICONTROL metrica Ciclo di vita] è stata rimossa dalle pagine [!UICONTROL Caratteristiche] [!UICONTROL e Caratteristiche] cartella.
...
* È stato risolto un problema che [!UICONTROL causava il mancato caricamento del report] Cronologia file in uscita in alcune situazioni (AAM -45713).
* È stato risolto un problema che causava l&#39;aggiunta della caratteristica selezionata in precedenza al [!UICONTROL Generatore] di segmenti quando si faceva clic [!UICONTROL su Aggiungi caratteristica] in un campo vuoto (AAM -45599).
...
Aggiornato il 15 aprile 2019 per la versione di aprile 13

| Funzione | Descrizione |
| -----------| ---------- | 
| Cerca campagne | Nel selettore dell&#39;intervallo di date, il calendario è ora organizzato da domenica a sabato. Precedentemente era stata organizzata da domenica a sabato. |
| Rapporti | The Portfolio Report now includes the optional column _Portfolio Spend Strategy_. |
...
* È stato migliorato il flusso di lavoro quando si eliminano le risorse nel Compositore esperienza visivo. Deleted assets are now removed from the Offers library and from Scene7 (if applicable)Deleted assets no longer display in search results. (TGT-31981)
...
AEM 6.2 SP1–CFP19 (6.2.1.19), rilasciato il 07 marzo 2019, è un aggiornamento importante che include importanti correzioni per i clienti, introdotte successivamente alla data di disponibilità generale di AEM 6.2 SP1 nel dicembre 2016.
...
AEM 6.3 SP 3-CFP 3 (6.3.3.3), rilasciato il 14 marzo 2019, è un aggiornamento importante che include il rilascio corretto del cliente, rilasciato successivamente alla data di disponibilità generale di AEM 6.3 aprile 2017.
