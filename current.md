...
| Funzione | Descrizione |
| -----------| ---------- |
| [!UICONTROL Analisi]percorso cliente: Backfill [!UICONTROL automatizzato del set di dati] | Questa nuova opzione consente di importare tutti i dati storici per una connessione in [!UICONTROL Customer Journey Analytics]. (Documentazione da seguire) |
...
| [!UICONTROL Area di lavoro]: Creare automaticamente tabelle a forma libera da uno stato vuoto | In precedenza, non era possibile rilasciare componenti direttamente in un progetto vuoto o in un pannello vuoto; prima era necessario aggiungere una tabella a forma libera. Ora è possibile rilasciare i componenti direttamente in un progetto o pannello vuoto e una tabella a forma libera sarà automaticamente creata in un formato consigliato. Sono stati inoltre apportati miglioramenti al modo in cui i tipi di componenti misti (ad esempio, dimensioni e metriche) vengono gestiti quando vengono rilasciati insieme in una tabella a forma libera vuota. |

#### Correzioni nell&#39;analisi

* È stato risolto un problema che causava la mancanza di dati del segmento di Analytics in Audience Manager. (AN-206221)
* È stato risolto un problema con l&#39;elaborazione di [!UICONTROL Origini] dati che causava la visualizzazione di date errate. (AN-213604)
* È stato corretto un problema a causa del quale i file di classificazione non venivano caricati correttamente sull&#39;FTP. (AN-214102)
* È stato risolto un problema con il metodo API che `Segments.Get` non restituiva una risposta completa. (AN-206210)
* È stato risolto un problema che causava la conversione degli elementi di riga di tabella in caratteri speciali nel download [!DNL Workspace] PDF. (AN-196153)
* È stato risolto un problema con la chiamata API 1.4 di Adobe Analytics che `visattrcustomeridcustomerattributes` non funzionava correttamente. (AN-186873)
* È stato risolto un problema che causava la visualizzazione di dati nei report ma non presenti nel Feed dati. (AN-211923)
* È stato corretto un problema a causa del quale non era possibile copiare le autorizzazioni Profilo  prodotto. (AN-211113)
* È stato risolto un problema che impediva agli utenti con Federated ID di accedere al Generatore di report. (AN-207750)
...
* È stato risolto un problema che impediva la visualizzazione dei dati di classificazione nella visualizzazione con tendenze. (AN-212761)
* È stato risolto un problema che causava un numero di segmenti pubblicati errato in [!UICONTROL Segment Manager]. (AN-213374)
...
| -----------| ---------- | ---------- |
| Modifica della modalità di calcolo delle [!UICONTROL voci/uscite] in [!UICONTROL Workspace] | 7 aprile 2020 | In [!UICONTROL Analysis Workspace], a partire da marzo 2020, è stato modificato il modo in cui il valore _Nessuno_ interagisce con [!UICONTROL Entrate/Uscite]. Poiché ora è possibile attivare e disattivare _i numeri_ in [!UICONTROL Analysis Workspace], dopo l’entrata o l’uscita viene applicato il valore _Nessuno_ , mentre per le eVar prima era applicato. Ad esempio, si supponga che il primo hit di una visita non contenga alcun valore per le eVar, ma anche per il secondo. In [!UICONTROL Reporting e analisi] verrà visualizzato come _Non specificato_ per la voce, ma in [!UICONTROL Analysis Workspace] verrà visualizzato come valore per il secondo hit. |
