...
| Privacy Reporting: Analytics Admin Console | Se si abilita la funzione di reporting sulla privacy per Analytics, viene aggiunto un set di variabili riservate a una suite di rapporti.  Le variabili sono progettate per assistere nella raccolta dei dati di consenso dei consumatori a livello di hit.<br/>Nuove dimensioni:<br/><ul><li>Rifiuto gestione consenso</li><li>Consenso della gestione</li><li>Variabili di gestione del consenso: `[Link to new Consent Variables page in Analytics]()`</li></ul> |
...
* Quando si modificano le destinazioni per i tipi di pubblico personalizzati per Twitter, il selettore [!UICONTROL Account] ora è attivo solo se alla destinazione non è assegnato un [!DNL Twitter Ads] account (AAM-49975).
