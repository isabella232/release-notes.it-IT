...
* Aggiorna publishDestinations in base alle nuove modifiche API.
...
Per la documentazione sul prodotto, consulta il servizio [Experience Cloud ID](https://docs.adobe.com/content/help/en/id-service/using/home.html).
...
* È stato risolto un problema in cui il nome/cognome dell’utente (migrato) di Analytics era diverso da quello collegato all’Adobe ID dell’utente in Experience Cloud. Per aggiornare il nome di Analytics con quello di Experience Cloud, vai su **[!UICONTROL Amministratore]** > **[!UICONTROL Gestione utente (legacy)]** > **[!UICONTROL Modifica utenti]** e fai clic sull&#39;ID utente. Fai clic su **[!UICONTROL Update from Experience Cloud]** (Aggiorna da Experience Cloud). (AN-172825)
...
| [Limiti di mappatura ID](https://marketing.adobe.com/resources/help/en_US/aam/usage-limits.html) | Abbiamo migliorato la segmentazione, il reporting e l’esportazione di segmenti limitando il numero mappature ID attraverso dispositivi diversi associate all’ID di un dispositivo. I nuovi limiti imposti ci aiutano a mitigare l’impatto di integrazioni errate e insiemi di dati non accurati sull’assimilazione e sull’elaborazione a valle. I nuovi limiti sono:<ul><li> 100 ID dispositivo per ID multi-dispositivo</li><li>10 ID multi-dispositivo per ID dispositivo</li><li> 1000 ID dispositivo a 1 ID dispositivo</li></ul> |

**Correzioni**

* Abbiamo migliorato la logica di segmentazione per le caratteristiche onboarded per allinearle alle caratteristiche basate su regole. (AAM-40433)
...
| Funzione | Descrizione |
| -----------| ---------- |  
| Compositore esperienza visivo | I seguenti miglioramenti introdotti nel Compositore esperienza visivo permettono di lavorare in modo più rapido ed efficiente:<ul><li>Ora puoi annullare il caricamento di un sito web nel Compositore esperienza visivo per sbloccare la modifica di un’attività. Questo miglioramento è utile, ad esempio, se desideri apportare una piccola modifica all’attività, rivederne le impostazioni o aggiungere codice personalizzato e non desideri attendere il caricamento del sito. Le azioni che non possono essere modificate prima del caricamento del sito vengono disabilitate nell’interfaccia utente di Target. (TGT-31288, TGT-31611 e TGT-32602)</li><li>Il Compositore esperienza visivo visualizza la struttura DOM, in modo da poter selezionare facilmente l’elemento corretto durante la creazione o la modifica delle esperienze. (TGT-13422)</li></ul> |

**Miglioramenti, correzioni, modifiche**

Ora viene richiesto di ripetere l’autenticazione alla scadenza della sessione durante la revisione di un rapporto. Dopo aver effettuato di nuovo l&#39;accesso, vieni reindirizzato al report. (TGT-32723)
...
### Miglioramenti al reporting dei canali push
...
Queste metriche consentono di misurare e analizzare l’interazione degli utenti con le notifiche push in modo più efficace. Stiamo inoltre standardizzando la definizione e l’implementazione di tali metriche. Il rapporto integrato della notifica push è stato migliorato anche con visualizzazioni e metriche di uso comune.

Per ulteriori informazioni, consulta [Rapporto notifiche push](https://helpx.adobe.com/it/campaign/standard/reporting/using/push-notification-report.html).
...
Per ulteriori informazioni, consulta [Utilizzo dell&#39;SDK di Adobe Experience Platform](https://helpx.adobe.com/it/campaign/standard/administration/using/configuring-a-mobile-application.html#main-pars_header_49).

### Messaggistica in-app mobile

Questa versione contiene la versione GA del canale in-app in Campaign. Dal punto di vista funzionale, le aggiunte più importanti alla versione beta sono i rapporti dinamici per il canale in-app e l’handshake sicuro tra Mobile SDK e MCIAS (Experience Cloud In-App Messaging Service, che funge da criterio in-app per l’SDK). L’handshake sicuro garantisce che i dati PII degli utenti non cadano in mani malintenzionate e consente di mantenere la privacy degli utenti su un dispositivo condiviso cancellando la cache dei messaggi ogni volta che l’utente si disconnette.

Per ulteriori informazioni, consulta [Informazioni sulla messaggistica in-app](https://helpx.adobe.com/it/campaign/standard/channels/using/about-in-app-messaging.html) e il tutorial [In-App](https://helpx.adobe.com/it/campaign/kt/acs/using/acs-in-app-message-tutorial.html) dedicato.
...
* **** Le attività Query e  **** Segmentazione ora consentono di abilitare una transizione in uscita se le attività non recuperano dati.
...
Consulta le [Note sulla versione](https://docs.adobe.com/content/help/it-IT/experience-manager-cloud-manager/using/release-notes/release-notes-current.html) associate.
...
L’edizione di Brand Portal 6.4.4 è ottimizzata per migliorare l’esperienza di ricerca delle risorse degli utenti. Si concentra sulla ricerca di frasi parziali e con caratteri jolly tramite il predicato delle proprietà e le richieste principali dei clienti.
...
* È stato risolto un problema che causava un errore di &quot;account duplicato&quot; durante l’aggiornamento del token dell’account aziendale di Instagram.
* È stato corretto un problema a causa del quale l’aggiornamento delle app tramite uno script personalizzato generava per errore app duplicate.
* È stato risolto un problema a causa del quale il contenuto Instagram visualizzato dalla ricerca URL mostra correttamente la miniatura nei risultati della ricerca ma non in &quot;Visualizza dettagli&quot; o pubblicato in un’app.
* È stato risolto un problema che causava un errore quando si faceva clic su Carica altro .
