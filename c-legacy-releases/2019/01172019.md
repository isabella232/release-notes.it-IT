...
* Personalizza la dimensione della coorte (per raggruppare i visitatori in base a un eVar , non solo in base al tempo).
...
* Amministratore: È stato risolto un problema che si verificava durante la creazione di una nuova suite di rapporti basata su un&#39;altra. Sono state copiate alcune impostazioni impreviste. (AN-172395)
...
* È stato corretto un problema a causa del quale i rapporti a cui si accedeva tramite collegamenti brevi e che contenevano le e commerciale nel filtro di ricerca perdevano alcune informazioni sui filtri. (AN-167151)
* È stato risolto un problema che causava visualizzazioni vuote nei pannelli dei rapporti scaricabili. (AN-171273)
* È stato risolto un problema che causava la mancata consegna dei report HTML pianificati contenenti confronti di date. (AN-168990)
* È stato risolto un problema che impediva la modifica dei segmenti. (AN-172307, AN-169785)
* È stato risolto un problema relativo alla richiesta di numero di telefono nell&#39;interfaccia utente della richiesta di impennata del traffico. Il campo del numero di telefono è stato rimosso. (AN-170992)
...
* È stato risolto un problema che impediva agli utenti di salvare le modifiche alle suite di rapporti virtuali associate a segmenti eliminati. (AN-168906)
* È stato risolto un problema che causava la cessazione del funzionamento delle metriche di partecipazione con le variabili di elenco in  Ad Hoc Analysis. (AN-166027)
...
* È stato effettuato un aggiornamento all&#39;interfaccia utente del Audience Marketplace  per consentire l&#39;immissione di valori decimali con virgola per le impression per Payables. (AAM-42994)
* È stato corretto un bug in  Audience Marketplace, in cui i clienti che desideravano effettuare autorapporti sui volumi delle impression per dicembre avrebbero trovato le impression di novembre ancora visibili nei campi di reporting. (AAM-43814)
...
* Supporto per variabili e campi di metadati in PDF e AEM Nomi sito.
...
* La colonna Checkout non veniva visualizzata correttamente  visualizzazione elenco AEM Assets.
...
* AEM Editor XML crea un errore nelle colonne in orizzontale.
...
| Visualizzazione mobile in Designer e-mail | È ora possibile passare a una vista mobile dedicata durante la modifica del contenuto delle e-mail. Ciò consente di mettere a punto il design reattivo di un&#39;e-mail modificando separatamente tutte le opzioni di stile per la visualizzazione mobile, come l&#39;adattamento dei margini, le dimensioni più piccole dei caratteri, i diversi colori di sfondo e così via. Per ulteriori informazioni, consulta la documentazione del prodotto. |
