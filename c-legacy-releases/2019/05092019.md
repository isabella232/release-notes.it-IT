...
* See [[!DNL Experience Platform Launch]](https://docs.adobe.com/content/help/it-IT/launch/using/overview.html) for the latest information.

### Servizio Experience Cloud ID

Rilascio: 13 **maggio 2019**

* Supporto API 4.3.0 per visitatori
* Supporto ITP 2.1.
...
| **Analysis Workspace:** Nuova impostazione con visualizzazione di flusso _Includi istanze di ripetizione_ | L’impostazione _Includi flusso istanze_ ripetute consente di includere o escludere istanze ripetute, ad esempio i ricarichi di pagina. Inoltre, tutte le visualizzazioni Flusso ora si basano solo sulle istanze. |
| **Ad Hoc Analysis:** Compatibilità con Java 11 | Ad Hoc Analysis è ora compatibile con Java 11. Learn how to run [Ad Hoc Analysis on Java 11](https://marketing.adobe.com/resources/help/en_US/dsc/adhoc-java.html). |
| **Raccolta dati:** Nuovo cookie s_ecid | Added a new [first-party server cookie](https://docs.adobe.com/content/help/it-IT/core-services/interface/ec-cookies/cookies-analytics.html), s_ecid, in which data collection stores the visitor&#39;s ECID. |

**Problemi risolti in Analysis Workspace**

* È stato corretto un problema che influisce sul tempo trascorso sulla pagina. I report Workspace non utilizzeranno più Nome pagina per il calcolo dei periodi fissi Time Spent, consentendo il conteggio degli hit granulari e buck. (AN-140479)
* Sono stati risolti i problemi di prestazioni della visualizzazione Linea nell’ambito di un maggiore sforzo per migliorare le prestazioni di Workspace. (AN-174878)
* È stato risolto un problema di mancanza di codifica UTF-8 nei file.csv scaricati. (AN-178393)
* Sono stati risolti i problemi relativi alle prestazioni rallentate del progetto Workspace. (AN-177710)
* Sono stati corretti i problemi di visualizzazione della visualizzazione Linea con piccoli intervalli nella granularità dell’asse y. (AN-176467)

**Altre correzioni apportate ad Analytics**

* Pubblico  Analytics: È stato risolto un problema che si verificava dopo la modifica del nome di un&#39;audience in  Audience Manager (AAM). Il nome aggiornato non veniva riflesso in Audience  Analytics. (AN-176237)
* È stato risolto un problema che impediva agli utenti di salvare  segmenti Analytics in AAM. Ciò è stato causato dalle cartelle AAM esistenti con nomi composti di caratteri maiuscoli e minuscoli. Ora tutte le cartelle vengono considerate senza distinzione tra maiuscole e minuscole in modo da poterle sincronizzare. (AN-177934)
* È stato risolto un problema che si verificava quando gli utenti accedevano a  Analytics tramite l&#39;Experience Cloud  e quindi la sessione terminava. Quando si riprende la sessione, l&#39;utente è stato reindirizzato a un URL errato. (AN-176812)
...
| Funzione | Descrizione |
| -----------| ---------- |  
| [Obfuscamento indirizzo IP](https://marketing.adobe.com/resources/help/en_US/aam/ip-obfuscation.html) | La tua azienda potrebbe voler oscurare l&#39;indirizzo IP in molti paesi a causa delle normative globali sulla privacy.  Audience Manager consente di oscurare gli indirizzi IP dei visitatori a livello globale o nazionale. |
| [Integrazioni partner personalizzate - Oracle Data Cloud](https://marketing.adobe.com/resources/help/en_US/aam/custom-partner-integrations.html) | Questa pagina elenca le integrazioni personalizzate tra Audience Manager e partner di dati.  Audience Manager trasferisce i dati dei cookie e dell&#39;ID mobile da Oracle Data Cloud per  Audience Marketplace tramite i file di dati in entrata. Le specifiche di integrazione personalizzata descritte in questa pagina fanno riferimento solo ai file di dati in entrata che contengono ID dispositivo mobile (IDFA e ID dispositivo Android). |
...
AEM 6.5, disponibile dall’8 aprile 2019, è un upgrade della codebase di AEM 6.4. I nostri ultimi aggiornamenti ad AEM 6.5 consentono di accedere immediatamente a miglioramenti accattivanti che consentono di progredire ancora più rapidamente.

* [Novità  Adobe Experience Manager 6.5](https://www.adobe.com/marketing/experience-manager/new.html)
...
L’ultima versione di Cloud Manager (2019.4.0, rilasciata il 18 aprile 2019) aggiunge un’interfaccia utente localizzata in francese, tedesco e giapponese. Sono stati inoltre migliorati i passaggi di distribuzione.
...
AEM 6.4, Service Pack 4 (6.4.4.0 rilasciato il 04 aprile 2019) è un aggiornamento importante che include correzioni di problemi fondamentali per i clienti, introdotte successivamente alla data di disponibilità generale di AEM 6.4, aprile 2018.

[Note sulla versione per le versioni di AEM Forms](https://helpx.adobe.com/it/experience-manager/6-4/release-notes/sp-release-notes.html)AEM 6.4 Service Pack[](https://helpx.adobe.com/it/aem-forms/kb/aem-forms-releases.html)

**Connettore AEM S3**

Le istanze AEM con versioni precedenti del connettore S3 Datastore potrebbero non essere disponibili a causa di errori di accesso S3 dopo la fine del supporto per la versione 2 della firma il 24 giugno 2019. In qualità di cliente AEM, Adobe consiglia di verificare la versione del connettore S3 Datastore in uso. Se necessario, eseguite l&#39;aggiornamento a una versione recente.

Vedere [L&#39;impatto di AWS Firma versione 2 Obsoleto per Amazon S3](https://helpx.adobe.com/it/experience-manager/kb/the-impact-of-aws-signature-version-2-deprecation-for-amazon-s3.html)

### Aiuto e documentazione

**Modernizza la base di codice per gli AEM Sites**

Scopri come sfruttare la tecnologia AEM più recente per modernizzare la base di codice per gli AEM Sites. [Modernizzazione Del Codebase Di Siti  Adobi Experience Manager Esistente](https://expleague.azureedge.net/labs/L761/index.html)

**Editor Rich Text di AEM - Dive profonde**

Scopri le best practice relative alle configurazioni avanzate e all’utilizzo dell’Editor Rich Text in AEM.

Consultate Dive profondi di [AEM Rich Text Editor (RTE)](https://helpx.adobe.com/it/experience-manager/kt/eseminars/gems/AEM-Rich-Text-Editor-RTE-Deep-Dive1.html)
...
| Funzione | Descrizione |
| -----------| ---------- |  
| Strumenti di ricerca | (Annunci con account Google Ads)  Advertising Cloud può facoltativamente caricare su Google Ads tutti i dati di conversione che tiene traccia per le campagne Google Ads che utilizzano il  servizio di tracciamento conversione Advertising Cloud. I caricamenti giornalieri includono il valore di conversione definito utilizzando il modello di attribuzione a livello di inserzionista. Tutte le conversioni caricate sono precedute da “Adobe_ACS_” (come “Adobe_ACS_Subscriptions” per la conversione “Subscriptions”). <br/> **Nota:** gli upload non includono i dati di conversione caricati su Advertising Cloud dai file di feed. |
...
| --- | --- |
| Compositore esperienza visivo per app a pagina singola (SPA VEC) | Il Compositore di esperienze visive SPA include i seguenti miglioramenti per permetterti di lavorare in modo più rapido ed efficiente:<ul><li>Ora potete annullare il caricamento di un sito Web nel VEC per sbloccare la modifica di un&#39;attività. Questo miglioramento è utile, ad esempio, se desiderate apportare una piccola modifica all&#39;attività, controllarne le impostazioni o aggiungere codice personalizzato e non desiderate attendere il caricamento del sito. (TGT-33872)</li><li>Potrai eseguire molte azioni prima che la pagina si carichi nel Compositore esperienza visivo, anche qualora non possa essere caricata completamente (ad esempio, se il codice personalizzato non è più operativo). Le azioni che non possono essere modificate prima del caricamento del sito vengono disattivate nell’interfaccia utente di Target. (TGT-33851 e TGT-34149)</li></ul> |
| Attività di Automated Personalization (AP) e Auto-Target | Potete selezionare un&#39;esperienza da utilizzare come controllo durante la creazione di un&#39;attività AP o Auto-Target. Questa funzione consente di indirizzare l&#39;intero traffico di controllo a un&#39;esperienza specifica, in base alla percentuale di allocazione del traffico configurata nell&#39;attività. Potete quindi valutare le prestazioni delle consegne personalizzate in base all&#39;esperienza di controllo. (TGT-26572) |
| Consigli | È possibile utilizzare l’opzione Consiglia articoli acquistati in precedenza durante la creazione della logica Articoli visualizzati di recente. (TGT-34030) |
...
* Ora potete elencare e navigare più facilmente tra le cartelle delle offerte nel selettore delle risorse, invece di navigare in una gerarchia di cartelle semplice. (TGT-33725)
