...
| Funzione | Descrizione |
| -----------| ---------- |
| Quadro di riferimento per la trasparenza e il consenso del RGPD | Advertising Cloud aggiorna il supporto per il Transparency and Consent Framework di IAB alla versione 2.0 entro il 15 agosto 2020. Al momento, non sono necessarie modifiche da parte del cliente.  In qualità di Incaricato del trattamento dei dati per ciascuno dei nostri clienti, Adobe Advertising Cloud dispone delle capacità disponibili per i clienti per adempiere ai loro obblighi ai sensi del RGPD applicabili all’uso dei nostri prodotti e servizi. |
| Integrazione di Audience Manager | (Beta solo per i clienti Adobe Audience Manager con Adobe Experience Platform) La DSP può inviare file di registro fruibili contenenti i dati di coinvolgimento pubblicitario di un inserzionista in Audience Manager tramite Adobe Experience Platform. In Audience Manager, gli inserzionisti possono utilizzare i dati per la segmentazione avanzata e l’utilizzo in strumenti di insights, come i rapporti di Audience Optimization. |
...
| Annunci flash | DSP ora inserisce automaticamente le macro di clic per tutti i tag degli annunci Flashtalk tramite i flussi di lavoro di creazione di annunci singoli e di creazione di annunci in massa di terze parti. |
| Rapporti | (Beta solo per i clienti Adobe Experience Platform) Un nuovo rapporto cross channel in Advertising Cloud Search fornisce una visualizzazione unificata della spesa media in DSP e Search, con metriche chiave giornaliere per posizionamenti DSP e campagne di ricerca. |
