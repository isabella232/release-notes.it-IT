...
* Aggiornamenti Dimension tra per segmenti sequenziali. Come la logica Instance Between (Istanza tra) che identifica le corrispondenze tra i contenitori, il Dimension Between (Tra) ora include una funzione di restrizione delle dimensioni che fornisce una logica basata sui valori delle dimensioni, determinando le corrispondenze in base a valori specifici (righe di tabella) per monitorare se una dimensione selezionata è impostata (o meno) e quindi fornire la logica dei segmenti basata su questi valori di dimensione.
